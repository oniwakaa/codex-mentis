...
from pitagora.cli.rich_ui import print_concept_map, print_panel, print_table
...
def load_all_concepts() -> dict[str, list[dict[str, Any]]]:
    ...
    for domain, names in DEFAULT_CONCEPTS.items():
        ...
        for idx, name in enumerate(names):
            ...
            fallback[domain].append({"id": cid, "name": name, "prerequisites": prereq})
    return fallback
...
@app.command("map")
def show_map(
    concept_id: str | None = typer.Argument(None, help="Specific concept ID to draw map for"),
    domain: str | None = typer.Option(None, "--domain", "-d", help="Domain to filter by"),
):
...
@app.command("status")
def show_status(domain: str | None = typer.Option(None, "--domain", "-d", help="Filter by domain")):
...
def list_concepts(
    ...
    """List all available concepts, optionally filtered by domain."""
    concepts_data = load_all_concepts()

    headers = ["Domain", "Concept Name", "Concept ID", "Prerequisites"]
    rows = []
    for dom, items in concepts_data.items():
        if domain and dom != domain:
            continue
        for item in items:
            prereqs = ", ".join(item.get("prerequisites", []))
            rows.append([dom, item["name"], item["id"], prereqs or "-"])

    print_table(headers, rows, title="Available Concepts")
...
def suggest_next(
    ...
    if not suggested:
        print_panel(
            "Congratulations! You have mastered all topics or no recommendations fit your current state.",
...
def show_review_queue():
    ...
    cursor.execute(
        "SELECT concept_id, mastery_score, last_reviewed FROM concept_mastery WHERE mastery_score < 0.8"
    )
    rows = cursor.fetchall()
    conn.close()
    ...
    # Map ID to metadata
    id_map = {}
    for _dom, items in concepts_data.items():
        for item in items:
            id_map[item["id"]] = item
