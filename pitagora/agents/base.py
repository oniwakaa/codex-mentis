...
class BaseAgent:
    ...
    def _validate_tool_args(self, tool_name: str, args: dict[str, Any]) -> str | None:
        """
    ...
    def think(self, prompt: str, context: str | None = None) -> AgentResponse:
        """
    ...
    async def athink(self, prompt: str, context: str | None = None) -> AgentResponse:
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                await self.events.emit(
                    "status_change", f"Querying provider (attempt {attempt+1}/{max_retries+1})..."
                )
                raw_response = await self.provider.acomplete(
                    messages=messages, tools=self.tools if self.tools else None, temperature=0.7
                )
                break
            except Exception as e:
                last_error = e
                ...
                await self.events.emit(
                    "status_change", f"Provider error: {e}. Retrying in {sleep_time:.2f}s..."
                )
        ...
        response = AgentResponse(
            content=content, tool_calls=tool_calls, confidence=confidence, metadata=metadata
        )
    ...
    async def athink_structured(
        self, prompt: str, response_model: type[BaseModel], context: str | None = None
    ) -> BaseModel:
        """
        ...
        for _attempt in range(max_attempts):
            response = await self.athink(prompt=structured_prompt, context=context)
        ...
        raise ValueError(
            f"Failed to generate structured output conforming to {response_model.__name__}: {last_exception}"
        )
    ...
    def _calculate_confidence(self, content: str, raw_response: dict[str, Any]) -> float:
        ...
        if "confidence" in raw_response:
            ...
            if conf_match:
                ...
                for word in ["unsure", "not certain", "maybe", "hypothesize"]
            ):
                confidence = 0.7
            elif any(
                word in content.lower()
                for word in ["cannot determine", "insufficient information", "contradiction"]
            ):
                confidence = 0.4
        return confidence
