...
def test_concept_graph_optimized_path(temp_yaml):
    ...
    path = cg.get_optimized_path(
        "Quantum Mechanics", mastered_concepts=["Calculus", "Linear Algebra"]
    )
...
def test_concept_graph_add_concept(temp_yaml):
    cg = ConceptGraph(yaml_path=temp_yaml)
    cg.add_concept(
        "Quantum Computing", ["Quantum Mechanics"], "Introduction to qubits", "Physics", 5, 120
    )
