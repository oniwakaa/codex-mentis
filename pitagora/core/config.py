...
class MathConfig(BaseModel):
    ...
    verification_levels: list[str] = Field(
        default_factory=lambda: ["computational", "cross_check"],
        description="Math verification levels",
    )
...
class MCPConfig(BaseModel):
    ...
    obsidian: str = Field(
        default_factory=lambda: str(Path("~/obsidian-vault").expanduser()),
