    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for entry in root.findall("atom:entry", ns):
                ...
                papers.append({"title": title_text, "summary": summary_text, "url": url_text})
            return papers
...
@app.command()
def explore(
    question: str = typer.Argument(None, help="What question or topic to explore?"),
    depth: str = typer.Option(
        "standard", "--depth", "-d", help="Research depth (surface/standard/deep)"
    ),
    papers: bool = typer.Option(
        False, "--papers", "-p", help="Search ArXiv academic repository for context"
    ),
    continue_journey: str = typer.Option(
        None, "--continue", "-c", help="Resume an existing journey by ID"
    ),
):
    ...
    if continue_journey:
        ...
        journey = load_journey(continue_journey)
        if journey is None:
            typer.echo(f"Error: No journey found with id '{continue_journey}'.")
            raise typer.Exit(1)
        typer.echo(f"Resuming journey: '{journey.topic}' (status: {journey.status})")
        launch_repl(
            mode="EXPLORE",
            topic=journey.topic,
        )
    ...
    if not question:
        typer.echo("Error: Provide a topic to explore, or use --continue <JOURNEY_ID> to resume.")
        raise typer.Exit(1)
