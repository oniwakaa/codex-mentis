...
from pitagora.agents import AgentResponse, BaseAgent
...
def test_base_agent_tool_registration(mock_provider):
    agent = BaseAgent(
        name="TestBase", role="Tester", provider=mock_provider, system_prompt="You are a tester."
    )
...
async def test_base_agent_think(mock_provider):
    agent = BaseAgent(
        name="TestBase", role="Tester", provider=mock_provider, system_prompt="System prompt"
    )
