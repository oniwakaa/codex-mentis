...
def review_start(
    ...
    if not due:
        console.print(Panel(
            ...
            "[dim]Use `codex-mentis study <topic>` to add new concepts to your review deck.[/dim]",
            title="📚 Review Status",
            border_style="green",
        ))
        return
    ...
    if total > 0:
        ...
        console.print(Panel(
            ...
            f"{'Excellent work! Keep it up.' if pct >= 80 else 'Keep practicing — it gets easier with repetition.' if pct >= 50 else 'Consider revisiting these concepts with `codex-mentis study`.'}",
...
def review_status():
    ...
    if review_count > 0:
        ...
        console.print("[dim]Run `codex-mentis review start` to begin.[/dim]")
    else:
