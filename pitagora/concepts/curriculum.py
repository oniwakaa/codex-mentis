...
class CurriculumGenerator:
    ...
    def suggest_next(self) -> dict[str, Any]:
        ...
        if roots_unmastered:
            ...
            return {
                "concept": suggested,
                "reason": reason,
                "details": self.concept_graph.graph[suggested],
            }
    ...
    def generate_study_plan(self, target: str, timeline_weeks: int = 4) -> list[dict[str, Any]]:
        ...
        mastered_list = [
            c for c in self.concept_graph.graph.keys() if self.tracker.get_mastery(c) >= 0.8
        ]

        # Use optimized pathfinder
        needed_concepts = self.concept_graph.get_optimized_path(
            target, mastered_concepts=mastered_list
        )

        if not needed_concepts:
            return [
                {
                    "message": f"You have already mastered the target concept '{target}' and all its prerequisites."
                }
            ]
