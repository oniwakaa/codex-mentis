...
class CodexMentisConfig(BaseModel):
...
def get_default_config() -> CodexMentisConfig:
    return CodexMentisConfig()

def ensure_config_dir() -> None:
    if not CONFIG_DIR.exists():
        CONFIG_DIR.mkdir(parents=True, exist_ok=True)

def load_config() -> CodexMentisConfig:
    ensure_config_dir()
    ...
    try:
        ...
        return CodexMentisConfig(**data)
    except Exception:
        # Fallback to default config on load error
        return get_default_config()

def save_config(config: CodexMentisConfig) -> None:
    ensure_config_dir()
