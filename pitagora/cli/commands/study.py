...
def check_prerequisites(topic: str) -> list[str]:
    ...
    from pitagora.cli.commands.concept import load_all_concepts
...
def study(
    ...
    domain: str = typer.Option(
        "physics", "--domain", "-d", help="Domain classification (math/physics)"
    ),
    difficulty: int = typer.Option(
        2, "--difficulty", "-g", min=1, max=5, help="Difficulty level (1-5)"
    ),
):
