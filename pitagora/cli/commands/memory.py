...
def show_memory(
    layer: str | None = typer.Option(
        None, "--layer", "-l", help="Specific layer to show (L1/L2/L3)"
    )
):
    ...
    if layer:
        cursor.execute(
            "SELECT id, layer, content, topic, timestamp FROM memory_entries WHERE layer = ? ORDER BY timestamp DESC",
            (layer.upper(),),
        )
    else:
        cursor.execute(
            "SELECT id, layer, content, topic, timestamp FROM memory_entries ORDER BY layer, timestamp DESC"
        )
...
def search_memory(query: str = typer.Argument(..., help="Query to search memory for")):
    ...
    if not semantic_success:
        ...
        cursor.execute(
            "SELECT id, layer, content, topic, timestamp FROM memory_entries WHERE content LIKE ? OR topic LIKE ? ORDER BY timestamp DESC",
            (f"%{query}%", f"%{query}%"),
        )
...
def clear_memory(
    layer: str = typer.Option(
        "L1", "--layer", "-l", help="Which memory layer to clear (L1/L2/L3 or ALL)"
    ),
...
def export_memory(
    output_file: Path | None = typer.Option(
        None, "--output", "-o", help="Target markdown file path"
    )
):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT layer, content, topic, timestamp FROM memory_entries ORDER BY layer, timestamp DESC"
    )
    rows = cursor.fetchall()
    conn.close()
    ...
    md_content = "# Pitagora - Exported Memory Vault\n\n"
    current_layer = None
