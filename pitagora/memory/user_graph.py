...
class UserGraph:
    ...
    def add_node(self, node_id: str, node_type: str, properties: dict | None = None) -> str:
        """Add a node to the graph, preserving created_at on conflict."""
    ...
    def add_edge(
        ...
        """Add an edge between two nodes, preserving created_at on conflict."""
        now = datetime.now().isoformat()
        conn = sqlite3.connect(self.db_path)
        conn.execute(
            """INSERT INTO graph_edges (source, target, edge_type, weight, properties, created_at)
               ...
               ON CONFLICT(source, target, edge_type) DO UPDATE SET
                 ...
                 created_at = graph_edges.created_at""",
    ...
    def get_neighbors(
        self, node_id: str, edge_types: list[str] | None = None, direction: str = "outgoing"
        ...
        for edge_row in edges:
            edge = GraphEdge(
                ...
                weight=edge_row[4],
                properties=json.loads(edge_row[5]),
                created_at=edge_row[6],
            )
            neighbor_id = edge.target if edge.source == node_id else edge.source
            node_row = conn.execute(
                "SELECT * FROM graph_nodes WHERE id = ?", (neighbor_id,)
            ).fetchone()
            if node_row:
                node = GraphNode(
                    id=node_row[0],
                    node_type=node_row[1],
                    properties=json.loads(node_row[2]),
    ...
    def traverse(
        self, start_id: str, max_depth: int = 2, edge_filter: list[str] | None = None
    ) -> dict[str, Any]:
    ...
    def shortest_path(self, from_id: str, to_id: str, max_depth: int = 5) -> list[str] | None:
        ...
        while queue:
            ...
            for neighbor_node, _edge in neighbors:
    ...
    def recommend_next(self, user_id: str) -> list[dict[str, Any]]:
        ...
        recommendations = []
        topics = conn.execute(
            "SELECT id, properties FROM graph_nodes WHERE node_type = 'topic'"
        ).fetchall()
    ...
    def get_knowledge_gaps(self, user_id: str) -> list[dict[str, Any]]:
        ...
        mastered = set()
        rows = conn.execute(
            "SELECT target FROM graph_edges WHERE source = ? AND edge_type = 'mastered'", (user_id,)
        ).fetchall()
        for row in rows:
            mastered.add(row[0])
    ...
    def visualize_graph(self, user_id: str, max_nodes: int = 30) -> str:
        ...
        for target, edge_type, weight, node_type, _props_json in edges:
