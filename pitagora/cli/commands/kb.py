...
def add_document(
    ...
    subject: str = typer.Option(
        "general", "--subject", "-s", help="Subject classification (e.g. physics, math)"
    ),
    title: str | None = typer.Option(None, "--title", "-t", help="Custom title for the document"),
):
    ...
    if not chunks:
        typer.echo("Error: Ingested file has empty content.")
        raise typer.Exit(1)
    ...
    try:
        ...
        conn.commit()
        typer.echo(
            f"Successfully ingested '{doc_title}' ({len(chunks)} chunks, subject: {subject})"
        )
