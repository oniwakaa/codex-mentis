...
app = typer.Typer(help="Configure Codex Mentis providers and settings")
...
def run_setup(console=None, quick: bool = False):
    ...
    console.print(Panel(
        "[bold cyan]⚙️  Codex Mentis Setup Wizard[/bold cyan]\n\n"
        "Configure your AI providers, model preferences, and features.\n"
        "You can always re-run this with `codex-mentis setup`.",
        border_style="cyan",
    ))

    config_dir = Path("~/.codex-mentis").expanduser()
    ...
    console.print(Panel(
        ...
        "  codex-mentis                    Launch the interactive chat\n"
        "  codex-mentis onboard            Set up your learning profile\n"
        "  codex-mentis study \"calculus\"   Start learning\n"
...
def _verify_connection(config: Dict, model: str, console):
    ...
    try:
        ...
        console.print("  [dim]You can still use Codex Mentis — configure the provider later with `codex-mentis setup`[/dim]")
