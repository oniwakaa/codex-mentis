...
class SkillForge:
    ...
    def __init__(self, user_skills_dir: str | None = None):
...
class SkillEvolution:
    ...
    def select_prompt_variant(self, skill_name: str, variants: dict[str, str]) -> tuple[str, str]:
        ...
        for var_name, _template in variants.items():
            try:
                rows = list(
                    db["skill_usage"].rows_where(
                        "skill_name = ? AND variant = ?", [skill_name, var_name]
                    )
                )
            except Exception:
                rows = []
    ...
    def mutate_prompt(
        ...
        try:
            latest = list(
                db["skill_prompts"].rows_where(
                    "skill_name = ?", [skill_name], order_by="version DESC", limit=1
                )
            )
    ...
    def generate_skill_from_interaction(
        self, topic: str, interaction_log: list[dict[str, str]], provider: Any | None = None
    ) -> Skill:
        """
        Generates a new Skill definition from a successful interaction log.
        """
        transcript = "\n".join(
            [f"{msg['role'].upper()}: {msg['content']}" for msg in interaction_log]
        )
        ...
        return Skill(
            ...
            verification_strategies=(
                [f"Verify with formula: {f}" for f in formulas[:2]]
                if formulas
                else ["Verify using dimensional consistency"]
            ),
        )
    ...
    def get_performance_dashboard(self) -> dict[str, Any]:
        ...
        if not runs:
            return {"total_usage_count": 0, "overall_success_rate": 0.0, "skills": {}, "topics": {}}
