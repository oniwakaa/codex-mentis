...
class MockAgent:
    ...
    async def explain_concept(self, topic, level="beginner"):
        return AgentResponse(
            content=f"Explaining {topic}", tool_calls=[], confidence=0.9, metadata={}
        )

    async def feynman_explanation(self, topic):
        return AgentResponse(
            content=f"Feynman: {topic}", tool_calls=[], confidence=0.9, metadata={}
        )

    async def explain_level(self, topic, level="intermediate"):
        return AgentResponse(
            content=f"Explaining {topic} at {level}", tool_calls=[], confidence=0.9, metadata={}
        )
    ...
    async def research(self, topic, depth="medium"):
        return AgentResponse(
            content=f"Research on {topic}", tool_calls=[], confidence=0.8, metadata={}
        )

    async def derive(self, request):
        return AgentResponse(
            content=f"Derivation of {request}", tool_calls=[], confidence=0.95, metadata={}
        )

    async def review(self, claim):
        return AgentResponse(
            content=f"Review of {claim}", tool_calls=[], confidence=0.85, metadata={}
        )
