...
from pitagora.cli.rich_ui import create_spinner, format_proof, print_panel
...
def get_derivation_steps(request: str) -> list[tuple[str, str]]:
    ...
    if "euler" in req_lower or "lagrange" in req_lower or "least action" in req_lower:
        return [
            ...
            (
                "Apply variation: q(t) -> q(t) + epsilon * eta(t) where eta(t1)=eta(t2)=0",
                "boundary conditions",
            ),
            (
                "Compute variation: delta_S = Integral(diff(L, q)*delta_q + diff(L, q_dot)*delta_q_dot, (t, t1, t2))",
                "total variation",
            ),
            (
                "Express delta_q_dot as d/dt(delta_q) and integrate the second term by parts: Integral(diff(L, q_dot)*d/dt(delta_q)) = [diff(L,q_dot)*delta_q]_t1^t2 - Integral(d/dt(diff(L,q_dot))*delta_q)",
                "integration by parts",
            ),
            (
                "Boundary conditions eliminate boundary term: [diff(L,q_dot)*delta_q]_t1^t2 = 0",
                "boundary term vanishes",
            ),
            (
                "Collect terms: delta_S = Integral((diff(L,q) - d/dt(diff(L,q_dot))) * delta_q, (t, t1, t2)) = 0",
                "stationary action",
            ),
            (
                "For arbitrary delta_q (eta), the integrand must vanish: diff(L,q) - d/dt(diff(L,q_dot)) = 0",
    ...
    # 2. Quadratic Formula
        ...
        return [
            ...
            (
                "Complete the square: x**2 + (b/a)*x + (b/(2*a))**2 = (b/(2*a))**2 - c/a",
                "x**2 + (b/a)*x + b**2/(4*a**2) = b**2/(4*a**2) - c/a",
            ),
            (
                "Factor LHS: (x + b/(2*a))**2 = (b**2 - 4*a*c) / (4*a**2)",
                "(x + b/(2*a))**2 = (b**2 - 4*a*c)/(4*a**2)",
            ),
            (
                "Take square root: x + b/(2*a) = sqrt(b**2 - 4*a*c) / (2*a)",
                "x + b/(2*a) = sqrt(b**2 - 4*a*c)/(2*a)",
            ),
            (
                "Subtract b/(2*a): x = (-b + sqrt(b**2 - 4*a*c)) / (2*a)",
                "x = (-b + sqrt(b**2 - 4*a*c))/(2*a)",
            ),
        ]

    # 3. Simple algebraic check
    return [(f"Verify claim: {request}", request)]


@app.command()
def derive(
    request: str = typer.Argument(..., help="What equation or theorem to derive?"),
    verify_level: str = typer.Option(
        "computational",
        "--verify",
        "-v",
        help="Verification depth (computational/cross_check/formal)",
    ),
):
    ...
    for _idx, (desc, expr) in enumerate(steps, 1):
        # Verify step
    ...
    if has_error:
        print_panel(
            "One or more derivation steps failed symbolic verification. Revising proof steps...",
            "Proof Refinement",
            style="yellow",
        )
    ...
    if not has_error:
        print_panel(
            "Verification Complete. Confidence Score: 1.0 (Symbolically verified)",
            "Result",
            style="green",
        )
    else:
        print_panel(
            "Proof contains unverified algebraic transitions. Confidence Score: 0.6",
            "Result",
            style="red",
        )
