...
from pitagora.core.constants import KNOWLEDGE_GRAPH_DB
...
class KnowledgeGraph:
    def __init__(self, db_path: str = str(KNOWLEDGE_GRAPH_DB)):
    ...
    def _resolve_entity_id(self, name_or_id: str, strict: bool = False) -> str | None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id FROM entities WHERE (id = ? OR name = ?) AND is_deleted = 0",
                (slug, name_or_id),
            )
            row = cursor.fetchone()
            if row:
    ...
    def find_entity(self, name_or_id: str) -> EntityNode | None:
        slug = name_or_id.strip().lower().replace(" ", "_")
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def find_related(
        self, entity_id: str, rel_type: str | None = None, depth: int = 2
    ) -> list[tuple[EntityNode, Relationship, int]]:
        # Strict read-only: must not auto-create entities
    ...
    def semantic_search(self, query: str, limit: int = 5) -> list[EntityNode]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, name, entity_type, properties, embedding, created_at, updated_at FROM entities WHERE is_deleted = 0"
            )
            rows = cursor.fetchall()
        ...
        for row in rows:
            ...
            dot_product = sum(q * m for q, m in zip(query_vector, m_vector, strict=False))
    ...
    def graph_traversal(
        self, start_id: str, max_depth: int = 2, rel_types: list[str] | None = None
    ) -> dict[str, Any]:
        # Read-only traversal; strict mode prevents entity creation
    ...
    def merge_entities(self, id1: str, id2: str) -> str:
        ...
        # Use resolved entity IDs in all SQL operations
        rid1 = e1.id
        rid2 = e2.id

        now = datetime.datetime.now().isoformat()
        # Merge properties with precedence: e1 properties override e2 (or vice versa depending on semantics)
        # Using: keep both with e1 taking precedence for same keys
        merged_properties = {**e2.properties, **e1.properties}
        merged_properties_json = json.dumps(merged_properties)

        # Fetch relationships to migrate, using resolved IDs
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("PRAGMA foreign_keys = ON")
            conn.execute("PRAGMA busy_timeout = 30000")
            cursor = conn.cursor()
            cursor.execute(
                "SELECT target_id, rel_type, properties, weight FROM relationships WHERE source_id = ? AND is_deleted = 0",
                (rid2,),
            )
            source_rels = cursor.fetchall()
            cursor.execute(
                "SELECT source_id, rel_type, properties, weight FROM relationships WHERE target_id = ? AND is_deleted = 0",
                (rid2,),
            )
            target_rels = cursor.fetchall()

        # Migrate relationships using resolved IDs, avoiding duplicates
        for tgt, r_type, r_props_json, weight in source_rels:
            if tgt != rid1:
                self.add_relationship(
                    rid1, tgt, r_type, json.loads(r_props_json or "{}"), float(weight)
                )

        for src, r_type, r_props_json, weight in target_rels:
            if src != rid1:
                self.add_relationship(
                    src, rid1, r_type, json.loads(r_props_json or "{}"), float(weight)
                )

        # Soft-delete absorbed node and original relationships, and update merged properties
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("PRAGMA foreign_keys = ON")
            conn.execute("PRAGMA busy_timeout = 30000")
            cursor = conn.cursor()
            ...
            cursor.execute(
                "UPDATE entities SET is_deleted = 1, updated_at = ? WHERE id = ?",
                (now, rid2),
            )
            cursor.execute(
                "UPDATE relationships SET is_deleted = 1, updated_at = ? WHERE source_id = ? OR target_id = ?",
                (now, rid2, rid2),
            )
            conn.commit()
    ...
    def get_context_window(self, entity_id: str, max_tokens: int = 1000) -> str:
        # Read-only: do not create entities
        resolved_id = self._resolve_entity_id(entity_id, strict=True)
        if not resolved_id:
            return ""
        ...
        if related:
            ...
            for rel_entity, rel, _ in related:
                if rel.source_id == resolved_id:
                    lines.append(
                        f"  - {entity.name} --[{rel.rel_type} (weight: {rel.weight})]--> {rel_entity.name}"
                    )
                else:
                    lines.append(
                        f"  - {rel_entity.name} --[{rel.rel_type} (weight: {rel.weight})]--> {entity.name}"
                    )
    ...
    def temporal_query(
        self, entity_id: str, before: str | None = None, after: str | None = None
    ) -> list[dict[str, Any]]:
        resolved_id = self._resolve_entity_id(entity_id, strict=True)
        if not resolved_id:
            return []
        ...
        sql = (
            "SELECT source_id, target_id, rel_type, properties, weight, updated_at FROM relationships WHERE "
