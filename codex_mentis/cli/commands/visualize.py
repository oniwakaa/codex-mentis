...
from codex_mentis.cli.rich_ui import print_plot, print_panel, create_spinner

app = typer.Typer(help="Visualize mathematical expressions and functions")

@app.command("plot")
def plot_expression(
    expression: str = typer.Argument(..., help="Mathematical expression to plot (e.g. 'sin(x) * exp(-0.2*x)')"),
    x_range: Tuple[float, float] = typer.Option((0.0, 10.0), "--range", "-r", help="Min and Max bounds for independent variable"),
    plot_type: str = typer.Option("function", "--type", "-t", help="Plot type (function/scatter/parametric)"),
    save_path: Optional[Path] = typer.Option(None, "--save", "-s", help="Path to save the plot as an image (PNG/PDF)")
):
    ...
    try:
        with create_spinner("Generating numerical coordinates...") as status:
            if plot_type == "parametric":
                # Expecting format "f(t), g(t)"
                parts = expression.split(",")
                if len(parts) != 2:
                    typer.echo("Error: Parametric plots require x and y expressions separated by a comma (e.g. 'cos(t), sin(t)').")
                    raise typer.Exit(1)
